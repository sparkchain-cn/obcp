var $, tab, skyconsWeather,appid,element;
layui.config({
	base: "/js/"
	...
	$(".closePageOther").on("click", function() {
		if($("#top_tabs li").length > 2 && $("#top_tabs li.layui-this cite").text() != "后台首页") {
			...
			$("#top_tabs li").each(function() {
				if($(this).attr("lay-id") != 'welcome' && !$(this).hasClass("layui-this")) {
...
function getMenu(){
	...
	if(index == 0){
		appid = pathname.substring(5);
		return "/appNav?appid="+pathname.substring(5)+"&time="+Date.parse(new Date());
	}
	return "/navs?time="+Date.parse(new Date());
}
...
//创建新选项卡
function createTab(title,url,id){
	var filter = "bodyTab";
...
function closeTab(id) {
	var filter = "bodyTab";
	parent.element.tabDelete(filter, id);
}
