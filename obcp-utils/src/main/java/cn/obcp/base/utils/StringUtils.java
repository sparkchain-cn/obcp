...
public class StringUtils {
	...
	public static String mergeIds(String oldIds, String newIds) {
		...
		String[] both = (String[]) ListUtils.addAll(ids, newidss);
		return StringUtils.join(Arrays.asList(both), ",");
	}
	...
	public static String genFileName(String fileName, String pattern, boolean need) {
	...
	public static String join(Collection cs, String sep, int type) {
		...
		for (Object o : cs) {
			...
			if (type == 1) {
				s = singleQuote(o.toString()) + sep;
			} else if (type == 2) {
				s = quote(o.toString()) + sep;
			}
			sb.append(s);
		}
...
//		}
	 ...
	 * 
	 * @param path
	 * @param needhead false 如果路径中第一个字符是/,去掉它
	 * @return
	 */
	...
	public String delDotSepIds(String str, String ids, String splitchar) {
		...
		List<String> oldList = ListUtils.strToList(str, splitchar);
		...
		return ListUtils.listToStr(oldList);
